...
public class IodQuery_20150319_2 {
     ...
     * @param item_code
     */
    public IodQuery_20150319_2(String store_code, String barcode) {
...
//            sql.append("SELECT CASE WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0700' AND '0800' THEN 'F'"	);
            ...
            */
            ...
            sql.append("       		ELSE 'D' END DAY_CHECK,"																										  );
            sql.append("   	   CASE WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0000' AND '1500' OR TO_CHAR(SYSDATE,'HH24:MI') < '16:00' THEN TO_CHAR(SYSDATE,'yyyymmdd')");
            sql.append("       		ELSE TO_CHAR(SYSDATE+1,'yyyymmdd') "								);
            sql.append("       	END ORDER_DATE,"														);
            sql.append("       NVL('1600',0) AS ORDER_TIME,"											);
            sql.append("       CASE WHEN TO_CHAR(TRUNC(SYSDATE, 'MI'), 'HH24:MI') > '1600'  THEN 'O' "	);		// 설정된 발주 제한시간
            sql.append("    		WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0000' AND '1500' THEN 'O'"	);		// 새벽 제한 발주시간
            sql.append("    		ELSE 'U'"			);
            sql.append("    	END ORDER_TIME_FLAG"	);
            sql.append("  FROM STORE_SECTION_ORDERTIME"	);
            sql.append(" WHERE SECTION_CD = ?"			);
...
//            	throw new GotException("발주 불가능한 시간 입니다.\nAM 07:00~08:00 발주 리스트 생성 시간입니다");
            	throw new GotException("발주 불가능한 시간 입니다.\nAM 15:00~15:30 발주 리스트 생성 시간입니다");
            }
