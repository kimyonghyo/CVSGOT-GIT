...
public class IodSave_20150319_2 {
    ...
    public IodSave_20150319_2(String store_code, String barcode, String qty, String time_flag, String order_method, String user_id) {
	 ...
	 * @throws Exception
	 */
	public void executeQuery(Logger logger) throws GotException,Exception {
...
//            sql.append("SELECT CASE WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0700' AND '0800' THEN 'F'"		);
            sql.append("SELECT CASE WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '1500' AND '1530' THEN 'F'"		);
            sql.append("       		ELSE 'T'"																);
            sql.append("       	END ORDER_FALSE, "															);    
            sql.append("   	   CASE WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0000' AND '1500' OR TO_CHAR(SYSDATE,'HH24:MI') > '16:30' THEN 'DD'");            
            sql.append("       		ELSE 'D'"																);
            sql.append("       	END DAY_CHECK,"																);
            sql.append("       CASE WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0000' AND '1500' THEN TO_CHAR(SYSDATE,'yyyymmdd')"	);
            sql.append("       		ELSE TO_CHAR(SYSDATE+1,'yyyymmdd')"														  	);
            sql.append("       	END ORDER_DATE,"																				);            
            sql.append("       CASE WHEN TO_CHAR(TRUNC(SYSDATE, 'MI'), 'HH24:MI') > '1600' THEN 'O'"							);
            sql.append("            WHEN TO_CHAR(SYSDATE,'HH24MI') BETWEEN '0000' AND '1500' THEN 'O'"							);
            sql.append("       		ELSE 'U'"																					);
...
//            	throw new GotException("발주 불가능한 시간 입니다.\nAM 07:00~08:00 발주 리스트 생성 시간입니다.");
               	throw new GotException("발주 불가능한 시간 입니다.\nPM 15:00~15:30 발주 리스트 생성 시간입니다.");
             }
          ...
          //발주시간이 마감 되었다면
            if(this._time_flag.equalsIgnoreCase("O")){
         		...
         		*/
         		logger.writeEntry("분기 - 발주시간 마감 : UPDATE ORDER_MANUAL_FU 명령");
            ...
            else
            {
         		...
         		*/
         		...
         		logger.writeEntry("분기 - 발주시간 이내 : UPDATE ORDER_MANUAL 명령");
